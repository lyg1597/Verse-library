...
def compute_reachtube_mixmono_cont(
    ...
    trace = [[0]+init]
    r = ode(decomposition_dynamics).set_integrator('dopri5', nsteps=2000).set_initial_value(init).set_f_params(uncertain)
    for i in range(len(t)):
...
def calculate_bloated_tube_mixmono_cont(
    ...
    if hasattr(agent, 'dynamics') and hasattr(agent, 'decomposition'):
        ...
        res = compute_reachtube_mixmono_cont(init, uncertain_param, time_horizon, time_step, decomposition)
    else:
...
def calculate_bloated_tube_mixmono_disc(
        ...
        if hasattr(agent, 'dynamics') and hasattr(agent, 'decomposition'):
            ...
            def decomposition_func(x, w, xhat, what, dt):
                expr_func = lambda x, num_var, args, idx: agent.dynamics(list(x[:num_var]), list(x[num_var:])+args)[idx]
                jac_func = lambda x, num_var, args, idx: agent.dynamics_jac(list(x[:num_var]), list(x[num_var:])+args)[idx, :]
