from typing import List, Tuple, Dict, DefaultDict, Any, NamedTuple
import copy
import itertools
import functools
...
class Simulator:
    ...
    def postCont(
        ...
        for agent_id in node.agent:
            ...
            trace = node.agent[agent_id].TC_simulate(
                mode, init, remain_time, time_step, track_map)
    ...
    def postDisc(
        ...
        if not cache:
            ...
            if len(_transitions) > 0:
                ...
                for agent_id, segment in cache.items():
                    ...
                    cont_var_dict_template, discrete_variable_dict, len_dict = sensor.sense(self, agent, state_dict, track_map)
        ...
        for idx in range(trace_length):
            ...
            for agent_id in agent_guard_dict:
                ...
                continuous_variable_dict, orig_disc_vars, _ = sensor.sense(self, agent, state_dict, track_map)
                unchecked_cache_guards = [g[:2] for g in cached_guards[agent_id] if g[2] < idx]     # FIXME: off by 1?
                asserts, satisfied = check_sim_transitions(agent, agent_guard_dict[agent_id] + unchecked_cache_guards, continuous_variable_dict, orig_disc_vars, track_map, agent_state, agent_mode)
                if asserts != None:
    ...
    def simulate_one(
        ...
        for agent_id in node.agent:
            ...
            asserts, transitions, transition_idx = self.postDisc(node, transition_graph.map, transition_graph.sensor, new_cache, paths_to_sim)
    ...
    def simulate_simple(
        ...
        while simulation_queue != []:
            ...
            asserts, transitions, transition_idx = self.postDisc_Simple(node,transition_graph.map, transition_graph.sensor)
            # pp(("transitions:", transition_idx, transitions))
